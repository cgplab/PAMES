...
#' @param platform_data A data.frame with two colums (chromosome, genomic_coordinates).
...
#' @param control_costraints First element: value below which control upper quartile should be to select a given site;
#' second element: value above which control lower quartile should be to select a given site.
...
#' info_sites <- select_informative_sites(tumor_toy_data, auc_data, platform_data)
select_informative_sites_ext <- function(tumor_table, control_table, auc, platform_data,
    ...
    assertthat::assert_that(ncol(platform_data) >= 2)
    assertthat::assert_that(is.character(platform_data[[1]]))
    assertthat::assert_that(is.numeric(platform_data[[2]]))
    assertthat::assert_that(nrow(tumor_table) == nrow(platform_data), msg="Number of rows of tumor_table is not equal to the number of rows of platform_data.")
